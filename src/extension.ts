...
import { parseToSnapshot, computeBehavioralDiff, type BehavioralDiff } from './analysis';

const VIEW_TYPE = 'frontendFlowDiff.panel';

function getBeforeSource(uri: vscode.Uri): string | null {
...
function getWebviewContent(diff: BehavioralDiff, options: { includeCopy: boolean; includeApi: boolean; includeRoutes: boolean; includeStates: boolean }): string {
  const s = (arr: unknown[]) => (arr.length > 0 ? arr : null);
...
<body>
  ...
  <div id="content">
    ...
    <h2>Navigation</h2>
    <ul>
      ${(routesAdded as typeof diff.routes.added)?.map((r) => `<li class="added">Added: ${r.kind} → <code>${escapeHtml(r.path)}</code></li>`).join('') ?? ''}
      ${(routesRemoved as typeof diff.routes.removed)?.map((r) => `<li class="removed">Removed: ${r.kind} → <code>${escapeHtml(r.path)}</code></li>`).join('') ?? ''}
      ${(routesChanged as typeof diff.routes.changed)?.map((r) => `<li class="changed"><code>${escapeHtml(r.before.path)}</code> → <code>${escapeHtml(r.after.path)}</code></li>`).join('') ?? ''}
    </ul>` : '',
            (options.includeApi && (apiAdded || apiRemoved || apiChanged)) ? `
    <h2>API interactions</h2>
    <ul>
      ${(apiAdded as typeof diff.apiCalls.added)?.map((a) => `<li class="added">Added: ${a.method} <code>${escapeHtml(a.path)}</code></li>`).join('') ?? ''}
      ${(apiRemoved as typeof diff.apiCalls.removed)?.map((a) => `<li class="removed">Removed: ${a.method} <code>${escapeHtml(a.path)}</code></li>`).join('') ?? ''}
      ${(apiChanged as typeof diff.apiCalls.changed)?.map((a) => `<li class="changed"><code>${escapeHtml(a.before.path)}</code> → <code>${escapeHtml(a.after.path)}</code></li>`).join('') ?? ''}
    </ul>` : '',
            (options.includeStates && (statesAdded || statesRemoved || statesChanged)) ? `
    <h2>UI states</h2>
    <ul>
      ${(statesAdded as typeof diff.uiStates.added)?.map((s) => `<li class="added">Added: ${escapeHtml(s.label ?? s.condition)}</li>`).join('') ?? ''}
      ${(statesRemoved as typeof diff.uiStates.removed)?.map((s) => `<li class="removed">Removed: ${escapeHtml(s.label ?? s.condition)}</li>`).join('') ?? ''}
      ${(statesChanged as typeof diff.uiStates.changed)?.map((s) => `<li class="changed">${escapeHtml(s.before.condition)} → ${escapeHtml(s.after.condition)}</li>`).join('') ?? ''}
    </ul>` : '',
            (options.includeCopy && (copyAdded || copyRemoved || copyChanged)) ? `
    <h2>Copy</h2>
    <ul>
      ${(copyAdded as typeof diff.copy.added)?.map((c) => `<li class="added">Added: "${escapeHtml(c.text)}"</li>`).join('') ?? ''}
      ${(copyRemoved as typeof diff.copy.removed)?.map((c) => `<li class="removed">Removed: "${escapeHtml(c.text)}"</li>`).join('') ?? ''}
      ${(copyChanged as typeof diff.copy.changed)?.map((c) => `<li class="changed">"${escapeHtml(c.before.text)}" → "${escapeHtml(c.after.text)}"</li>`).join('') ?? ''}
    </ul>` : '',
          ].join('')
    }
  </div>
...
export function activate(context: vscode.ExtensionContext) {
  function runDiff(beforeSource: string, afterSource: string, filename: string) {
    const before = parseToSnapshot(beforeSource, filename);
    const after = parseToSnapshot(afterSource, filename);
    const diff = computeBehavioralDiff(before, after);
    const config = vscode.workspace.getConfiguration('frontendFlowDiff');
    const options = {
      includeCopy: config.get<boolean>('includeCopyChanges', true),
      includeApi: config.get<boolean>('includeApiCalls', true),
      includeRoutes: config.get<boolean>('includeRoutes', true),
      includeStates: config.get<boolean>('includeUiStates', true),
    };
  ...
  const diffCurrentFile = vscode.commands.registerCommand(
    'frontendFlowDiff.diffCurrentFile',
    () => {
      ...
      if (beforeSource === null) {
        vscode.window.showErrorMessage(
          'Could not get HEAD version. Is this file in a Git workspace and committed?'
        );
        return;
      }
  ...
  const showFromDiffEditor = vscode.commands.registerCommand(
    'frontendFlowDiff.showBehavioralDiffFromDiffEditor',
    () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
      ...
      if (beforeSource === null) {
        vscode.window.showErrorMessage(
          'Could not get HEAD version. Run from a Git workspace with the file committed, or use "Compare with HEAD" and then run this command.'
        );
        return;
      }
      runDiff(beforeSource, afterSource, filename);
