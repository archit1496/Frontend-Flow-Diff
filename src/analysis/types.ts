...
export interface BehavioralDiff {
  routes: {
    added: RouteChange[];
    removed: RouteChange[];
    changed: { before: RouteChange; after: RouteChange }[];
  };
  apiCalls: {
    added: ApiCall[];
    removed: ApiCall[];
    changed: { before: ApiCall; after: ApiCall }[];
  };
  uiStates: {
    added: UiState[];
    removed: UiState[];
    changed: { before: UiState; after: UiState }[];
  };
  copy: {
    added: TextNode[];
    removed: TextNode[];
    changed: { before: TextNode; after: TextNode }[];
  };
}
